...
public:
    BPlusTree(unsigned minimum_degree) :
...
private:
    void _remove_from_node(BPlusTree::Node_ptr node, unsigned index) {
    ...
    void _printNodeData(OStream &output, const std::shared_ptr<BPlusNode<DataType>> &node, int level) {
        if (!node) {
            output << "*\n";
            return;
        }
    ...
    template<typename OStream>
    void _printChildren(OStream &output, const std::shared_ptr<BPlusNode<DataType>> &node, int level) {
        ...
        for (auto &child : node->children) {
            _printStep(output, child, level + 1);
        }
    }

    template<typename OStream>
    void _printStep(OStream &output, const std::shared_ptr<BPlusNode<DataType>> &node, int level) {
        output << '|';
        for (int i = 0; i < level; i++) {
            output << '\t' << '|';
        }

        _printNodeData(output, node, level);
        _printChildren(output, node, level);
...
private:
    ...
    _subtree_search(std::shared_ptr<BPlusNode<DataType>> subtree_root, const DataType &key) {
